 ...
 * all-clear it did not earn. Write the escape sequence instead — a backslash
 * followed by x00, or by u001b for an escape byte. It is the same value to the
 * compiler and stays readable to every tool.
 */
...
function sourceFiles(dir: string, out: string[] = []): string[] {
  for (const name of readdirSync(dir)) {
    ...
    // Only source is in scope — asset/data files (images, fixtures) are not.
...
describe('source hygiene', () => {
  it('has no literal control bytes in src/ or tests/', () => {
    ...
    expect(offenders, 'write escape sequences instead of raw control bytes').toEqual([]);
  });
});
