...
describe('host election — nobody hosts a mesh that has not formed', () => {
  ...
  it('falls back to an election if the room turns out to have no host', async () => {
    ...
    //
    // The window is deliberately generous (SETTLE_MS = 6s, 3x the 2s announce
    // interval): the old 2.5s was routinely shorter than Nostr discovery plus
    // ICE on mobile, so a joiner elected ITSELF while the incumbent's first
    // announce was still in flight, and then won the min-id coin flip and took
    // a live room. Waiting a beat longer is what makes that impossible.
