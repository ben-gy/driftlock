...
export function createNet(config: NetConfig, handlers: NetHandlers = {}): Net {
  ...
  // trystero 0.21 constrains makeAction payloads to its DataPayload type; our
  // control channels are plain JSON, so bridge through a permissive signature.
  // Bound to `room`: the alias is called free-standing below, and any
  // implementation that reads `this` (the test doubles do) would break silently.
  const makeAction = room.makeAction.bind(room) as unknown as <T>(
    name: string,
  ) => [(d: T, to?: PeerId | PeerId[]) => void, (cb: (d: T, from: PeerId) => void) => void];
  ...
  const [sendHost, getHost] = makeAction<{ host: PeerId }>('__h');
  ...
  const [sendPing, getPing] = makeAction<{ t: number; id: string; pong?: boolean }>('ping');
  getPing((msg, from) => {
    if (msg.pong) {
